<?php
/**
 * Import WISDOM SCHOOL KANZENZE only (WIS-KAN / school 39).
 *
 * Source: deploy/_wisdom_kanzenze_school_information.json (parsed from
 * C:\methode\15 Wisdoms\9.Wisdom KANZENZE). Classes are REB
 ...
 * Staff photos are extracted from the Excel STAFF sheet PICTURE column.
 *
 * Usage:
 *   php deploy/import_wisdom_kanzenze_school_information.php --dry-run
 *   php deploy/import_wisdom_kanzenze_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_kanzenze_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_kanzenze')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Kanzenze import JSON\n");
	exit(1);
}

const TARGET_ACRONYM = 'WIS-KAN';
...
function match_photo(array $photos, string $fullName, array $usedFiles = []): ?array
{
	...
	foreach ($photos as $photo) {
		...
		if (stripos($file, 'whatsapp') !== false) {
			continue;
		}
...
if (!$school || (int) $school['id'] !== TARGET_SCHOOL_ID) {
	fwrite(STDERR, "Target Kanzenze school was not found at expected school id 39 / WIS-KAN\n");
	exit(1);
}
$schoolName = compact_name((string) ($school['name'] ?? ''));
if (strpos($schoolName, 'KANZENZE') === false || strpos($schoolName, 'SUSA') !== false
	...
	|| strpos($schoolName, 'KABARORE') !== false || strpos($schoolName, 'RWANDA') !== false) {
	fwrite(STDERR, "Refusing to import: school id 39 is not Wisdom Kanzenze (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL KANZENZE', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? 'FEARING GOD IS KNOWLEDGE', 100),
	'address' => 'Kanzenze, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'JEREMIE SEMAGOLI', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolkanzenze.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's39.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolkanzenze.rw';
		}
