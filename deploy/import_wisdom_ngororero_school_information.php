<?php
/**
 * Import WISDOM SCHOOL NGORORERO only (WIS-NGO / school 41).
 *
 * Source: deploy/_wisdom_ngororero_school_information.json (parsed from
 * C:\methode\15 Wisdoms\1.Wisdom Ngororero). Classes are REB
 ...
 *
 * Usage:
 *   php deploy/import_wisdom_ngororero_school_information.php --dry-run
 *   php deploy/import_wisdom_ngororero_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_ngororero_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_ngororero')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Ngororero import JSON\n");
	exit(1);
}

const TARGET_ACRONYM = 'WIS-NGO';
const TARGET_SCHOOL_ID = 41;
...
if (!$school || (int) $school['id'] !== TARGET_SCHOOL_ID) {
	fwrite(STDERR, "Target Ngororero school was not found at expected school id 41 / WIS-NGO\n");
	exit(1);
}
$schoolName = compact_name((string) ($school['name'] ?? ''));
if (strpos($schoolName, 'NGORORERO') === false || strpos($schoolName, 'SUSA') !== false
	...
	fwrite(STDERR, "Refusing to import: school id 41 is not Wisdom Ngororero (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL NGORORERO', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? '', 100),
	'address' => 'Ngororero, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'MANIRAFASHA Alphonse', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolngororero.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's41.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolngororero.rw';
		}
