#!/usr/bin/env python3
"""Deploy kiosk visitor roster (include parents without cards)."""
...
VERIFY_STRINGS = [
    ("app/Libraries/AttendanceScanService.php", "function visitorList"),
    ("app/Controllers/Api.php", "'student_id' => $studentId"),
    ("app/Controllers/Api.php", "'phone' => $visitor['phone'] ?? ''"),
]
...
{verify_cmds}
# Confirm the card-only roster filter is gone
if grep -n "TRIM(COALESCE(sv.card" "{REMOTE_BASE}/app/Libraries/AttendanceScanService.php"; then
  echo "UNEXPECTED card filter still present"
  exit 1
fi
echo DONE
"""
