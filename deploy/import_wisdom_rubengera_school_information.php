<?php
/**
 * Import WISDOM SCHOOL RUBENGERA only (WIS-RBE / school 33).
 *
 * Source: deploy/_wisdom_rubengera_school_information.json (parsed from
 * C:\methode\15 Wisdoms\5.Rubengera). Classes are REB
 ...
 *
 * Usage:
 *   php deploy/import_wisdom_rubengera_school_information.php --dry-run
 *   php deploy/import_wisdom_rubengera_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_rubengera_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_rubengera')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Rubengera import JSON\n");
	exit(1);
}

const TARGET_ACRONYM = 'WIS-RBE';
const TARGET_SCHOOL_ID = 33;
...
if (!$school || (int) $school['id'] !== TARGET_SCHOOL_ID) {
	fwrite(STDERR, "Target Rubengera school was not found at expected school id 33 / WIS-RBE\n");
	exit(1);
}

$schoolName = compact_name((string) ($school['name'] ?? ''));
if (strpos($schoolName, 'RUBENGERA') === false
	|| strpos($schoolName, 'SUSA') !== false
	|| strpos($schoolName, 'NYABIHU') !== false
	|| strpos($schoolName, 'BURERA') !== false
	|| strpos($schoolName, 'NGORORERO') !== false
	|| strpos($schoolName, 'MUYUMBU') !== false
	|| strpos($schoolName, 'RUNDA') !== false
	|| strpos($schoolName, 'KABARORE') !== false
	|| strpos($schoolName, 'KANZENZE') !== false
	|| strpos($schoolName, 'KIRAMURUZI') !== false
	|| strpos($schoolName, 'NYAMASHEKE') !== false
	...
	fwrite(STDERR, "Refusing to import: school id 33 is not Wisdom Rubengera (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL RUBENGERA', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? '', 100),
	'address' => 'Rubengera, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'SINDAYIHEBA FABIEN', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolrubengera.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's33.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolrubengera.rw';
		}
