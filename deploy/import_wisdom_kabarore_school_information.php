<?php
/**
 * Import WISDOM SCHOOL KABARORE only (WIS-KAB / school 42).
 *
 * Source: deploy/_wisdom_kabarore_school_information.json (parsed from
 * C:\methode\15 Wisdoms\8.WISDOM KABARORE). Classes are REB
 ...
 * Staff photos are named files (Name, Role.jpg) matched fuzzily by name tokens.
 *
 * Usage:
 *   php deploy/import_wisdom_kabarore_school_information.php --dry-run
 *   php deploy/import_wisdom_kabarore_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_kabarore_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_kabarore')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Kabarore import JSON\n");
	exit(1);
}

const TARGET_ACRONYM = 'WIS-KAB';
const TARGET_SCHOOL_ID = 42;
...
if (!$school || (int) $school['id'] !== TARGET_SCHOOL_ID) {
	fwrite(STDERR, "Target Kabarore school was not found at expected school id 42 / WIS-KAB\n");
	exit(1);
}
$schoolName = compact_name((string) ($school['name'] ?? ''));
if (strpos($schoolName, 'KABARORE') === false || strpos($schoolName, 'SUSA') !== false
	...
	fwrite(STDERR, "Refusing to import: school id 42 is not Wisdom Kabarore (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL KABARORE', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? '', 100),
	'address' => 'Kabarore, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'Innocent Benurwanda', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolkabarore.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's42.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolkabarore.rw';
		}
