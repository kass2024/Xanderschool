<?php
/**
 * Import WISDOM SCHOOL RUBAVU only (WIS-RUB / school 30).
 *
 * Source: deploy/_wisdom_rubavu_school_information.json (parsed from
 * C:\methode\15 Wisdoms\5.Rubavu). Classes are REB
 ...
 *
 * Usage:
 *   php deploy/import_wisdom_rubavu_school_information.php --dry-run
 *   php deploy/import_wisdom_rubavu_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_rubavu_school_information.json';
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Rubavu import JSON\n");
	exit(1);
}

const TARGET_ACRONYM = 'WIS-RUB';
...
if (!$school || (int) $school['id'] !== TARGET_SCHOOL_ID) {
	fwrite(STDERR, "Target Rubavu school was not found at expected school id 30 / WIS-RUB\n");
	exit(1);
}

$schoolName = compact_name((string) ($school['name'] ?? ''));
if (strpos($schoolName, 'RUBAVU') === false
	...
	fwrite(STDERR, "Refusing to import: school id 30 is not Wisdom Rubavu (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL RUBAVU', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? '', 100),
	'address' => 'Rubavu, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'MUVANDIMWE ANASTASE', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolrubavu.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's30.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolrubavu.rw';
		}
