 ...
 * Staff photos are camera files (TR AIMEEIMG_....jpg) matched by first name.
 *
 * Usage:
 *   php deploy/import_wisdom_runda_school_information.php --dry-run
 *   php deploy/import_wisdom_runda_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_runda_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_runda')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Runda import JSON\n");
	exit(1);
}
...
if (strpos($schoolName, 'RUNDA') === false || strpos($schoolName, 'SUSA') !== false
	...
	fwrite(STDERR, "Refusing to import: school id 31 is not Wisdom Runda (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL RUNDA', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? '', 100),
	'address' => 'Runda, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'DUSANGIYUMUKIZA GAD', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolrunda.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's31.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolrunda.rw';
		}
