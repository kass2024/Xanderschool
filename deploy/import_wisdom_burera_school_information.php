 ...
 *
 * Usage:
 *   php deploy/import_wisdom_burera_school_information.php --dry-run
 *   php deploy/import_wisdom_burera_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_burera_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_burera')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Burera import JSON\n");
	exit(1);
}
...
if (strpos($schoolName, 'BURERA') === false || strpos($schoolName, 'SUSA') !== false
	|| strpos($schoolName, 'NYABIHU') !== false || strpos($schoolName, 'RWANDA') !== false
	|| strpos($schoolName, 'KAYONZA') !== false) {
	fwrite(STDERR, "Refusing to import: school id 40 is not Wisdom Burera (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL BURERA', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? '', 100),
	'address' => 'Burera, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'HABANABAKIZE GILBERT', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolburera.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's40.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolburera.rw';
		}
