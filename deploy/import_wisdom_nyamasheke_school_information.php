<?php
/**
 * Import WISDOM SCHOOL NYAMASHEKE only (WIS-NYM / school 32).
 *
 * Source: deploy/_wisdom_nyamasheke_school_information.json (parsed from
 * C:\methode\15 Wisdoms\5.Nyamasheke). Classes are REB
 ...
 *
 * Usage:
 *   php deploy/import_wisdom_nyamasheke_school_information.php --dry-run
 *   php deploy/import_wisdom_nyamasheke_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_nyamasheke_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_nyamasheke')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Nyamasheke import JSON\n");
	exit(1);
}

const TARGET_ACRONYM = 'WIS-NYM';
const TARGET_SCHOOL_ID = 32;
...
function position_post(\CodeIgniter\Database\BaseConnection $db, string $position): int
{
	...
	if (strpos($position, 'DEPUTY') !== false) {
		return post_id($db, 'Deputy Head Teacher');
	}
	...
	if (strpos($position, 'ACCOUNTANT') !== false || strpos($position, 'BURSAR') !== false) {
		return post_id($db, 'Accountant');
	}
	if (strpos($position, 'COOK') !== false) {
		return post_id($db, 'Cook');
	}
	if (strpos($position, 'CLEAN') !== false) {
		return post_id($db, 'Cleaner');
	}
	if (strpos($position, 'GUARD') !== false) {
		return post_id($db, 'Guard');
	}
...
function token_close(string $a, string $b): bool
{
	...
	$a = str_replace(['PROMENSE', 'PROMESSE'], 'PROMES', $a);
	$b = str_replace(['PROMENSE', 'PROMESSE'], 'PROMES', $b);
	$a = str_replace(['INSHIMWE'], 'ISHIMWE', $a);
	$b = str_replace(['INSHIMWE'], 'ISHIMWE', $b);
	if ($a === $b) {
		return true;
	}
...
function name_key(string $value): string
{
	...
	$key = str_replace(['ALEXADRE', 'ALEXANDRE'], 'ALEXANDR', $key);
	$key = str_replace(['EMNMANUEL'], 'EMMANUEL', $key);
	$key = str_replace(['FRANSCISCO'], 'FRANCISCO', $key);
	$key = str_replace(['AKIMABA'], 'AKIMANA', $key);
	$key = str_replace(['PROMENSE'], 'PROMESSE', $key);
	$key = str_replace(['INSHIMWE'], 'ISHIMWE', $key);
	return $key;
}

function photo_hint_from_name(string $value): string
{
	$stem = pathinfo($value, PATHINFO_FILENAME);
	if ($stem === '' || $stem === '.' || $stem === $value) {
		$stem = $value;
	}
	$stem = preg_replace('/^TR\s*/i', '', $stem) ?? $stem;
	if (strpos($stem, ',') !== false) {
		$stem = explode(',', $stem, 2)[0];
	}
	$parts = preg_split('/IMG[_-]?\d/i', $stem);
	$hint = trim((string) ($parts[0] ?? $stem));
	$hint = preg_replace('/\b(P[1-6]|BABY|MIDDLE|TOP|CLSS|CLASS|TEACHER|HEADTEACHER|HEAD|SCHOOL|ACCOUNTANT|DOS)\b/i', ' ', $hint) ?? $hint;
	$hint = preg_replace('/[^A-Za-z ]/', ' ', $hint) ?? $hint;
	return trim(preg_replace('/\s+/', ' ', $hint) ?? '');
}

function token_similar(string $a, string $b): bool
{
	if (token_close($a, $b)) {
		return true;
	}
...
function collect_photos(string $photoDir, array $fromJson): array
{
	...
	foreach ($fromJson as $photo) {
		if (is_array($photo) && !empty($photo['file'])) {
			...
			$hint = photo_hint_from_name($file);
			if ($hint === '') {
				$hint = photo_hint_from_name((string) ($photo['name_hint'] ?? ''));
			}
			$photos[$file] = ['file' => $file, 'name_hint' => $hint];
		}
	}
	if (is_dir($photoDir)) {
		foreach (scandir($photoDir) ?: [] as $file) {
			...
			$hint = photo_hint_from_name($file);
			$photos[$file] = ['file' => $file, 'name_hint' => $hint !== '' ? $hint : pathinfo($file, PATHINFO_FILENAME)];
		}
	}
	return array_values($photos);
}

function match_photo(array $photos, string $fullName, array $usedFiles = []): ?array
{
	...
	foreach ($photos as $photo) {
		...
		$file = (string) ($photo['file'] ?? '');
		if ($file !== '' && isset($usedFiles[$file])) {
			continue;
		}
		$hint = photo_hint_from_name((string) ($photo['name_hint'] ?? $file));
		if ($hint === '') {
			$hint = photo_hint_from_name($file);
		}
		...
		if ($wantKey !== '' && $haveKey !== '') {
			if ($wantKey === $haveKey || strpos($wantKey, $haveKey) !== false || strpos($haveKey, $wantKey) !== false) {
				$score = 1.0;
				if ($score > $bestScore) {
					$bestScore = $score;
					$best = $photo;
		...
		if ($overlap >= 1 && (count($have) <= 2 || $overlap >= 2) && $score > $bestScore) {
			$bestScore = $score;
			$best = $photo;
		}
	}
...
if (!$school || (int) $school['id'] !== TARGET_SCHOOL_ID) {
	fwrite(STDERR, "Target Nyamasheke school was not found at expected school id 32 / WIS-NYM\n");
	exit(1);
}
$schoolName = compact_name((string) ($school['name'] ?? ''));
if (strpos($schoolName, 'NYAMASHEKE') === false || strpos($schoolName, 'SUSA') !== false
	|| strpos($schoolName, 'NYABIHU') !== false || strpos($schoolName, 'BURERA') !== false
	|| strpos($schoolName, 'NGORORERO') !== false || strpos($schoolName, 'MUYUMBU') !== false
	|| strpos($schoolName, 'RUNDA') !== false || strpos($schoolName, 'KAYONZA') !== false
	|| strpos($schoolName, 'KABARORE') !== false || strpos($schoolName, 'KANZENZE') !== false
	|| strpos($schoolName, 'KIRAMURUZI') !== false || strpos($schoolName, 'RWANDA') !== false) {
	fwrite(STDERR, "Refusing to import: school id 32 is not Wisdom Nyamasheke (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL NYAMASHEKE', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? '', 100),
	'address' => 'Nyamasheke, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'SAFARI JEAN PIERRE', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	$photoName = '';
	$matchedPhoto = null;
	$wantedFile = clean_text($staff['photo'] ?? '');
	if ($wantedFile !== '') {
		foreach ($photos as $photo) {
			if (!is_array($photo)) {
				continue;
			...
			$file = (string) ($photo['file'] ?? '');
			if ($file === $wantedFile && !isset($usedPhotos[$file])) {
				$matchedPhoto = $photo;
				break;
			}
		}
	...
	if (!$matchedPhoto) {
		$matchedPhoto = match_photo($photos, $fullName, $usedPhotos);
	}
	if (!$matchedPhoto) {
		say('PHOTO UNMATCHED ' . $fullName);
	}
	if ($matchedPhoto) {
		say('PHOTO MATCH ' . $fullName . ' <- ' . ($matchedPhoto['file'] ?? '') . ' [' . ($matchedPhoto['name_hint'] ?? '') . ']');
		$usedPhotos[(string) ($matchedPhoto['file'] ?? '')] = true;
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolnyamasheke.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's32.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolnyamasheke.rw';
		}
...
foreach ($data['classes'] as $student) {
	...
	$sex = strtoupper(clean_text($student['sex'] ?? 'U', 1));
	if ($sex !== 'M' && $sex !== 'F') {
		$sex = 'U';
	}
	$key = compact_name($fullName);
	$existing = $studentsByName[$key] ?? null;
	if ($existing && isset($usedStudentIds[(int) ($existing['id'] ?? 0)])) {
		$existing = null;
	}
