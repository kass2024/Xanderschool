 ...
 * Dispatch {LEVEL}A students into {LEVEL}A + {LEVEL}B, balancing gender, studying mode, and class size.
 *
 * Dry run:  docker exec xander_school_app php /var/www/html/deploy/dispatch_wisdom_primary_ab.php --level=P3
 * Execute:  docker exec xander_school_app php /var/www/html/deploy/dispatch_wisdom_primary_ab.php --level=P3 --execute
 */
...
$labelA = $levelCode . 'A';
$labelB = $levelCode . 'B';
say("=== Dispatch Wisdom {$labelA} -> {$labelA}/{$labelB} " . ($execute ? 'EXECUTE' : 'DRY RUN') . ' ===');
...
$secA = findSection($db, $levelCode, 'A');
$secB = findSection($db, $levelCode, 'B');
if (!$secA || !$secB) {
	fwrite(STDERR, "Missing {$labelA} or {$labelB} class for school " . SCHOOL_ID . "\n");
	exit(1);
}
say(sprintf('%s id=%d | %s id=%d', $labelA, $secA['id'], $labelB, $secB['id']));
...
if ($existingB > 0) {
	say("WARNING: {$labelB} already has {$existingB} student(s). Script only moves from {$labelA}; existing {$labelB} kept.");
}
...
$students = $db->query(
	...
	[$secA['id'], $yearKey, SCHOOL_ID]
)->getResultArray();

if ($students === []) {
	fwrite(STDERR, "No students in {$labelA} for this year.\n");
	exit(1);
}

say("{$labelA} source students: " . count($students));
[$keepA, $moveB] = balanceSplit($students);
summarize($keepA, "Plan {$labelA}");
summarize($moveB, "Plan {$labelB}");

say("--- Assignments to {$labelB} ---");
foreach ($moveB as $st) {
	say(sprintf(
...
foreach ($moveB as $st) {
	...
	if ($already) {
		...
		$db->query('UPDATE class_records SET class = ? WHERE id = ?', [$secB['id'], $crId]);
	}
	remapOnMove(
		$db,
		$sid,
...
say("DONE moved={$moved} | {$labelA} now={$cntA} | {$labelB} now={$cntB}");
exit(0);
