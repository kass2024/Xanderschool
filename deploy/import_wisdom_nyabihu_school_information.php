<?php
/**
 * Import WISDOM SCHOOL NYABIHU only (WIS-NYB / school 29).
 *
 * Source: deploy/_wisdom_nyabihu_school_information.json (parsed from
 * C:\methode\15 Wisdoms\10.Wisdom NYABIHU). Classes are REB
 ...
 *
 * Usage:
 *   php deploy/import_wisdom_nyabihu_school_information.php --dry-run
 *   php deploy/import_wisdom_nyabihu_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_nyabihu_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_nyabihu')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Nyabihu import JSON\n");
	exit(1);
}

const TARGET_ACRONYM = 'WIS-NYB';
const TARGET_SCHOOL_ID = 29;
...
if (!$school || (int) $school['id'] !== TARGET_SCHOOL_ID) {
	fwrite(STDERR, "Target Nyabihu school was not found at expected school id 29 / WIS-NYB\n");
	exit(1);
}
$schoolName = compact_name((string) ($school['name'] ?? ''));
if (strpos($schoolName, 'NYABIHU') === false || strpos($schoolName, 'SUSA') !== false
	|| strpos($schoolName, 'RWANDA') !== false || strpos($schoolName, 'KAYONZA') !== false) {
	fwrite(STDERR, "Refusing to import: school id 29 is not Wisdom Nyabihu (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolnyabihu.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's29.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolnyabihu.rw';
		}
