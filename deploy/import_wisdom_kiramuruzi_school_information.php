<?php
/**
 * Import WISDOM SCHOOL KIRAMURUZI only (WIS-KIR / school 36).
 *
 * Source: deploy/_wisdom_kiramuruzi_school_information.json (parsed from
 * C:\methode\15 Wisdoms\4.KIRAMURUZI). Classes are REB
 ...
 * Staff photos are WhatsApp portraits matched from the name caption on each picture.
 *
 * Usage:
 *   php deploy/import_wisdom_kiramuruzi_school_information.php --dry-run
 *   php deploy/import_wisdom_kiramuruzi_school_information.php
 */
...
$jsonPath = __DIR__ . '/_wisdom_kiramuruzi_school_information.json';
$dryRun = in_array('--dry-run', $argv ?? [], true);
$photoDir = rtrim((string) (getenv('PHOTO_DIR') ?: (WRITEPATH . 'staff_photos_import_kiramuruzi')), '/\\') . DIRECTORY_SEPARATOR;
...
if (!is_array($data) || !is_array($data['school'] ?? null)
	...
	fwrite(STDERR, "Invalid Kiramuruzi import JSON\n");
	exit(1);
}

const TARGET_ACRONYM = 'WIS-KIR';
const TARGET_SCHOOL_ID = 36;
...
if (!$school || (int) $school['id'] !== TARGET_SCHOOL_ID) {
	fwrite(STDERR, "Target Kiramuruzi school was not found at expected school id 36 / WIS-KIR\n");
	exit(1);
}
$schoolName = compact_name((string) ($school['name'] ?? ''));
if (strpos($schoolName, 'KIRAMURUZI') === false || strpos($schoolName, 'SUSA') !== false
	...
	fwrite(STDERR, "Refusing to import: school id 36 is not Wisdom Kiramuruzi (" . ($school['name'] ?? '') . ")\n");
	exit(1);
}
...
$schoolUpdate = [
	'name' => clean_text($schoolInfo['name'] ?? 'WISDOM SCHOOL KIRAMURUZI', 100),
	'slogan' => clean_text($schoolInfo['slogan'] ?? '', 100),
	'address' => 'Kiramuruzi, Rwanda',
	'head_master' => clean_text($schoolInfo['head_teacher'] ?? 'Habimana Emmanuel', 100),
	'updated_at' => $now,
];
if ($activeTerm) {
...
foreach ($data['staff'] as $staff) {
	...
	if ($existing) {
		...
		if ($email === '') {
			...
			$email = trim($slug, '.') . '@wisdomschoolkiramuruzi.rw';
		}
		$taken = $db->table('staffs')->where('email', $email)->get(1)->getRowArray();
		if ($taken) {
			$email = 's36.' . preg_replace('/[^a-z0-9]+/i', '.', strtolower($fullName)) . '@wisdomschoolkiramuruzi.rw';
		}
