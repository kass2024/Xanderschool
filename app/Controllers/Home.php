...
public function testEmail()
{
	...
	private function fetchClassForSchool(int $schoolId, int $classId): ?array
	{
		...
		$row = $classMdl->select("classes.id, classes.title, classes.level, classes.department,
			d.id as department_id, d.code, d.code as dept_code, l.id as level_id, l.title as level_name")
	...
	private function remapStudentRecordsOnClassMove(
		...
		// School fees / discounts: remap when level or department changes.
		$fromLevel = (int) ($fromClass['level_id'] ?? $fromClass['level'] ?? 0);
		$fromDept = (int) ($fromClass['department_id'] ?? $fromClass['department'] ?? 0);
		$toLevel = (int) ($toClass['level_id'] ?? $toClass['level'] ?? 0);
		$toDept = (int) ($toClass['department_id'] ?? $toClass['department'] ?? 0);
		if (
			...
			$oldFees = $db->query(
				'SELECT id, term, academic_year FROM school_fees
				 WHERE school_id = ? AND level = ? AND department = ? AND academic_year = ?',
				[$schoolId, $fromLevel, $fromDept, $yearKey]
			)->getResultArray();
			$newFees = $db->query(
				'SELECT id, term, academic_year FROM school_fees
				 WHERE school_id = ? AND level = ? AND department = ? AND academic_year = ?',
				[$schoolId, $toLevel, $toDept, $yearKey]
			)->getResultArray();
			$newByTerm = [];
			foreach ($newFees as $sf) {
				$term = (int) ($sf['term'] ?? 0);
				if ($term > 0 && !isset($newByTerm[$term])) {
					$newByTerm[$term] = (int) $sf['id'];
				}
			}
			foreach ($oldFees as $sf) {
				$oldId = (int) ($sf['id'] ?? 0);
				$term = (int) ($sf['term'] ?? 0);
				$newId = $newByTerm[$term] ?? 0;
				if ($oldId < 1 || $newId < 1 || $oldId === $newId) {
					continue;
				}
				$db->query(
					'UPDATE fees_records SET fees_id = ?
					 WHERE student_id = ? AND fees_type = 0 AND fees_id = ?',
					[$newId, $studentId, $oldId]
				);
				if ($db->tableExists('school_fees_discount')) {
					$db->query(
						'UPDATE school_fees_discount SET feesId = ?
						 WHERE student = ? AND feesId = ?',
						[$newId, $studentId, $oldId]
					);
				}
			}
		...
		// Pending registration / application class hint if present.
...
//		print_r($ext_due_dates); die();
	...
	public function save_live_student_photo()
	{
		...
		$saved = function_exists('save_profile_photo_white_bg_from_string')
			? save_profile_photo_white_bg_from_string($decoded, $profilePath . $name, false, 'cover', false)
			: false;
	...
	public function settings()
	{
		...
		$ttSchema->restorePrimaryNurseryHourPeriods($schoolId);
	...
	public function ped_scheme_of_work()
	{
		$hasReb = $this->schoolHasRebClasses();
		$sub = $hasReb
			? 'Generate REB Scheme of Work per subject and class — edit on web, save, download landscape PDF with school header.'
			: 'Select an extracted course and generate a weekly Scheme of Work mapped to the chronogram.';
		return $this->renderPedagogicalPage('scheme', 'Scheme of Work', $sub);
	}
	...
	private function schoolHasRebClasses(): bool
	{
		$schoolId = (int) $this->session->get('soma_school_id');
		if ($schoolId <= 0) {
			return false;
		}
		$row = (new ClassesModel())->select('classes.id')
			->join('departments d', 'd.id=classes.department')
			->join('faculty f', 'f.id=d.faculty_id')
			->where('classes.school_id', $schoolId)
			->where('f.type', FacultyModel::TYPE_REB)
			->get(1)->getRowArray();
		return !empty($row);
	}
	...
	{
		$school = (new SchoolModel())->select('id,name,acronym,address,phone,email,website,slogan,head_master,logo')
		...
		return [
			...
			'target_grade' => $this->detectRebGradeFromClass($class ?: []),
		];
	}

	/** @param array<string,mixed> $class */
	private function detectRebGradeFromClass(array $class): string
	{
		$hay = strtolower(trim((string) (($class['level_name'] ?? '') . ' ' . ($class['title'] ?? ''))));
	...
	private function runAiAnalyzeCurriculum(int $schoolId, int $classId, int $yearId, bool $force, array $ctx, int $createdBy = 0)
	{
		...
		if (is_dir($pkgDir)) {
			...
			foreach ($rii as $file) {
				...
				if (!in_array($e, ['pdf', 'doc', 'docx', 'jpg', 'jpeg', 'png', 'webp'], true)) {
					continue;
				}
...
public function assign_card()
{
	...
	private function parentVisitingProcessScan($schoolId, $cardRaw, $source = 'web', $operator = null)
	{
		...
		if (empty($activeVisitors) && $firstInactive) {
			$inactive = $this->parentVisitingFormatVisitor($firstInactive);
			return [
				'allowed' => false,
				'success' => false,
