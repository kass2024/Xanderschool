...
<div class="aiplan-wrap">
	...
	<div class="aiplan-card">
		...
		<p class="aiplan-busy mt-2" id="aiplanBusy"><i class="fa fa-spinner fa-spin"></i> Analysing curriculum + chronogram (LO/IC + weekly hours)…</p>
	</div>
...
(function ($) {
	...
	function busy(on) {
		...
		$('#btnAnalyzeCurriculum,#btnForceAnalyze').prop('disabled', !!on);
	}
	...
	function analyze(force) {
		...
		busy(true);
		status(force ? 'Re-analysing curriculum + chronogram (weekly hours)…' : 'Loading cache / analysing…');
		$.post('<?= base_url('ai_analyze_curriculum'); ?>', { class_id: cid, force: force ? 1 : 0 }, function (res) {
			...
			busy(false);
			status((xhr.responseJSON && xhr.responseJSON.error) || 'Analysis failed', true);
		});
	}
	...
	$('#btnForceAnalyze').on('click', function () {
		if (!confirm('Re-run AI and overwrite cache?')) return;
		analyze(true);
	});
})(jQuery);
</script>
