			...
			<div class="card tt-gen-card tt-gen-card-pro h-100">
				...
				<div class="card-body">
					...
					<div id="ttCriteriaBox" class="tt-criteria-box mb-3" hidden>
						...
						<p class="small text-muted mb-2">Document rules apply on high school (combined classes, Alice not Monday, teacher windows, PE last hour, mornings, clinical). Combined courses share one teacher period: document pairs (Chemistry, Biology, Math, Computer Science, Economics, Entrepreneurship) and parallel sections with the same teacher and weekly hours (S1 A + S1 B Mathematics uses 2 periods for both classes; both stay busy so nothing else can take that slot). Izabayo Patience: Tuesday full day and Friday after lunch only. Sunday is reserved: save a <strong>Teach on Sunday</strong> course rule first. Farming and Library and Clubs stay after 15:40 and never at night. Generation uses the periods already saved in school settings and never creates a new period from special criteria. Nursery and primary have no Sunday column.</p>
