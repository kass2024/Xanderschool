<div class="row mb-3">
<div class="col-lg-7">
<div class="card border-success mb-3"><div class="card-header bg-success text-white">Wisdom term budget</div><div class="card-body">
<a class="btn btn-outline-success mr-2" href="<?= base_url('budget/download_term_template'); ?>"><i class="fa fa-download"></i> Download term template</a>
<?php if (!empty($can_install_official)) { ?>
<button type="button" class="btn btn-warning" id="btnResetTerm"><i class="fa fa-redo"></i> Reset &amp; seed from template</button>
<?php } ?>
</div></div>
...
<div class="col-lg-5">
<div class="card"><div class="card-header">Upload Excel backup</div><div class="card-body">
...
$('#btnResetTerm').on('click',function(){
	var c=prompt('This deletes ALL budgets and cash requests for your branch.\nType RESET to confirm:');
	if(c!=='RESET'){return;}
	$.post('<?= base_url('budget/reset_term_budget'); ?>',{confirm:'RESET'},function(r){
		if(r.error){toastada.error(r.error);return;}
		toastada.success(r.success||'Done');
		location.href='<?= base_url('budget/dashboard'); ?>';
	},'json');
});
</script>
